namespace EisaAwards.ConsoleApp
{
    ...
    {
        private static AuditWorker auditWorker;
        private readonly Logic.ExternalLogic externalAudit;
        private readonly Logic.InternalLogic internalAudit;

        private AuditWorker(ref Microsoft.EntityFrameworkCore.DbContext dbContext)
        {
            this.externalAudit = new Logic.ExternalAuditLogic(ref dbContext);
            this.internalAudit = new Logic.InternalAuditLogic(ref dbContext);
            /*
            var brands = new BrandRepository(ref context);
            var countries = new CountryRepository(ref context);
            var expertgroups = new ExpertGroupRepository(ref context);
            var members = new MemberRepository(ref context);
            var products = new ProductRepository(ref context);
